...
type
  ...
  public
    { Public declarations }
    mPage,mSite,mProtocol,mPort:string;//主页URL ，站点URL, 协议(http://,https://),工作目录
    mPageIdx:integer;
    ...
    function addPageInfo(protocol,domain,port,url:string):integer;
    function UpdatePageInfo(protocol,domain,port,url:string):integer;
  end;
...
var
  mWorkDir:string;
//添加主页信息,,返回主页信息ID：
function TDM.UpdatePageInfo(protocol,domain,port,url:string):integer;
begin
...
//添加主页信息,,返回主页信息ID：
function TDM.addPageInfo(protocol,domain,port,url:string):integer;
begin
  if(dm.tPage.Active=false)then begin
    dm.tPage.Open;
    tPageDetail.Open;
